/**
 * Test harness for the **tunneled-MessagePort** cross-origin iframe-
 * broker sync RPC topology.
 *
 * Same architecture as `SyncRpcProxyIframeWorkerClient` — cross-origin
 * parent and iframe, SAB inside the same-origin iframe ↔ worker pair
 * — with one difference: the parent ↔ iframe upward transport is a
 * `MessagePort` that the parent transfers to the iframe by tunneling
 * it through the worker:
 *
 *   parent — sends `port2` via
 *            `iframe.contentWindow.postMessage(..., [port2])`
 *   iframe — receives port, forwards to worker for trust hop
 *   worker — receives port, re-transfers back via
 *            `self.postMessage(..., [port])`
 *   iframe — receives port back; uses as broker's upstream transport
 *
 * Origins use HTTPS (with `ignoreHTTPSErrors: true` on the context).
 */
...
const PARENT_ENTRY = resolve(ENTRIES_DIR, 'tunneled-parent.ts');
const IFRAME_ENTRY = resolve(ENTRIES_DIR, 'tunneled-iframe.ts');
...
export interface SyncRpcTunneledBrokerClientOptions {
...
export class SyncRpcTunneledBrokerClient {
  readonly page: Page;
  ...
  static async create(
    ...
    opts: SyncRpcTunneledBrokerClientOptions = {},
  ): Promise<SyncRpcTunneledBrokerClient> {
    ...
    return new SyncRpcTunneledBrokerClient(page, parentOrigin, iframeOrigin);
  }
